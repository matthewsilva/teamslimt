...
public class myGameScreen extends BasicGameState 
{
	...
	private final int bossShotTimer = 100;
	
	private ArrayList<Alien> aliens= new ArrayList();
	private ArrayList<Projectile> shots = new ArrayList();
	private ArrayList<Projectile> brainShots = new ArrayList();
	private ArrayList<Powerup> powerups = new ArrayList();
	
	private int invulnTime;
	private Player player;
	private Boss brain;
	private int brainInvulnTime;
...
public void render(GameContainer container, StateBasedGame game, Graphics g) throws SlickException 
{
	...
	//Draws the boss if it has spawned
	if (bossSpawned == true) {
   		brain.getAnimation().draw(brain.getXPos(),brain.getYPos());
	...
	//Draws the aliens
   	...
   	//Draws the boss's projectiles
   	for (Projectile shot : brainShots) 
   	{
...
public void update(GameContainer container, StateBasedGame game, int delta) throws SlickException {
	...
	if (movementTime >= frameTime) 
	{
		...
		updateObjectMovement(aliens, shots, brainShots);
		checkCollisions(aliens, powerups, brainShots, player);
		
		if (bossSpawned == true) {
			checkBoss(shots, brain);
	...
	//Keeps track of how long the Boss has been invincible
	if (bossSpawned == true && brain.isInvincible()) 
	{
		brainInvulnTime += delta;
	}
	...
	//Resets invincibility
	if (brainInvulnTime > brainInvulnlength) 
	{
		brain.setInvincible(false);
		brain.setInvulnLength((long) 1000);
		brainInvulnTime = 0;
	}
	
	//Spawn new boss
	if (score > 4 && bossKilled != true) {
	...
	//Modifies the boss movement if the boss is spawned
	if (bossSpawned == true) {
		brain.moveBoss();
		brain.checkBossPhase();
		for (int i = 0; i < shots.size(); i++)
		{
			/*if (shots.get(i).intersects(brain)) {
	...
	if (bossSpawned == true && bossShotTime >=  bossShotTimer) {
		brain.shootBoss(brainShots);
		bossShotTime = 0;
	}
	
	// Boss is killed
	if (bossSpawned == true && brain.getLives() <= 0) {
		bossSpawned = false;
		bossKilled = true;
		brain = null;
	}
...
public void updateObjectMovement (ArrayList<Alien> aliens, ArrayList<Projectile> shots, ArrayList<Projectile> bossShots)
{
		//Moves aliens
		for (Alien alien : aliens)
		{
		...
		//Moves the boss's Projectile
		for (Projectile shot : bossShots)
		{
			shot.checkPosition();
			shot.move();
		}
...
public void checkCollisions(ArrayList<Alien> aliens, ArrayList<Powerup> powerups, ArrayList<Projectile> bossShots, Player myPlayer)
{
	...
	for (Projectile shot : bossShots)
	{
		if (shot.intersects(myPlayer) && !myPlayer.isInvincible())
		{
			myPlayer.setLives(myPlayer.getLives() - 1);
			myPlayer.setInvincible(true);
		}
...
public void checkBoss(ArrayList<Projectile> shots, Boss brain) {
	for (Projectile shot : shots)
	{
		if (shot.intersects(brain) && !brain.isInvincible())
		{
			brain.setLives(brain.getLives() - 1);
			brain.setInvincible(true);
		}
	}
}
